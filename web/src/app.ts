...
import { bytes, clamp, degOf, fmtInt, ticksAgo } from './ui/format';
...
export class App {
  ...
  /** Exposed for the console: `rw.config()`. */
  currentConfig(): Config | null {
    return this.config ? cloneConfig(this.config) : null;
  }

  get lastChart(): number {
    return this.lastChartTick;
  }

  wasmBytesLabel(): string {
    return bytes(__WASM_BYTES__);
  }

  fovLabel(radians: number): string {
    return degOf(radians) + '°';
  }
}
